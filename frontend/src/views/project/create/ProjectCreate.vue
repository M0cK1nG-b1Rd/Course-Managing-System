...
<!--      添加项目成员信息-->
      <el-form-item >
        <div>
          ...
          <!-- 用户列表区域 -->
          <el-row>
            <el-table :data="formData.memberList" border stripe>
...
export default {
  ...
  methods: {
    ...
    // 按照学号删除成员信息
    removeUserBySno(sno){
      for(let i = 0; i<this.formData.memberList.length; i++){
        if(this.formData.memberList[i].sno == sno){
    ...
    addMemberCommit(info) {
      console.log('新用户的信息')
      // 向formData.memberList中添加新创建的成员信息
      console.log(info)
      console.log('添加前的formData.memberList')
      console.log(this.formData.memberList)
      this.formData.memberList.push(info)
      console.log('添加后formData.memberList')
      console.log(this.formData.memberList)
      // 通知添加成功
